...
from nn_detect_browning_proj import LRegressionModule, parse_metadata_proj
...
class SmoothIG_proj:
    ...
    def calculate_results(self, ig_augmentations, img, img_bl):
        heatmaps = []
        results = []
        baseline_results = []
        for ig_augmentation in tqdm(ig_augmentations):
            img_aug = self.augmentations(image=ig_augmentation.apply(img))["image"]
            if isinstance(img_bl, str) and img_bl == "zero":
                baseline_aug = torch.zeros_like(img_aug)
            else:
        ...
        return heatmap, results, baseline_results
    ...
    def run_sub_experiment(self, name, ig_augmentations):
        ...
        for pstack in self.pstacks:
            ...
            for i in np.round(np.linspace(0, projs_bl.shape[0], self.num_imgs, endpoint=False)).astype(int):
                ...
                heatmap, results, baseline_results = self.calculate_results(ig_augmentations, img, img_bl)
                ...
                heatmap_black, _, _ = self.calculate_results(ig_augmentations, img, np.zeros_like(img_bl))
                heatmap_zero, _, _ = self.calculate_results(ig_augmentations, img, "zero")

                v_long = np.percentile(np.abs(heatmap), 98)
                v_black = np.percentile(np.abs(heatmap_black), 98)
                v_zero = np.percentile(np.abs(heatmap_zero), 98)

                plt.figure(figsize=(16.5, 2.5))
                plt.subplot(151)
                ...
                plt.xticks(np.arange(0, 720, 200))
                plt.yticks(np.arange(0, 690, 200))
                plt.grid()
                plt.subplot(152)
                ...
                plt.grid()
                plt.subplot(153)
                plt.imshow(heatmap, vmin=-v_long, vmax=v_long, cmap="PiYG")
                plt.colorbar(label="Attribution to browning score")
                plt.title("IG (longitudinal)")
                plt.xticks(np.arange(0, 720, 200))
                plt.yticks(np.arange(0, 690, 200))
                plt.grid()
                plt.subplot(154)
                plt.imshow(heatmap_black, vmin=-v_black, vmax=v_black, cmap="PiYG")
                plt.colorbar(label="Attribution to browning score")
                plt.title("IG (black)")
                plt.xticks(np.arange(0, 720, 200))
                plt.yticks(np.arange(0, 690, 200))
                plt.grid()
                plt.subplot(155)
                plt.imshow(heatmap_zero, vmin=-v_zero, vmax=v_zero, cmap="PiYG")
                plt.colorbar(label="Attribution to browning score")
                plt.title("IG (zero)")
                plt.xticks(np.arange(0, 720, 200))
                plt.yticks(np.arange(0, 690, 200))
                plt.grid()
                plt.tight_layout()
                plt.savefig(scan_figures_folder / f"{pstack.apple_nr}_{i}.svg", dpi=300)
                plt.close()
        file.close()
