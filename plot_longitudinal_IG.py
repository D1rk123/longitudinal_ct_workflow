    ...
    def run_sub_experiment(self, name, ig_augmentations):
        ...
        for scan in self.scans:
            ...
            for slice_nr in np.round(np.linspace(min_slice_nr, max_slice_nr, self.num_imgs)).astype(int):
                ...
                heatmap, results, baseline_results = self.calculate_results(ig_augmentations, img, img_day0)
                ...
                heatmap_black, _, _ = self.calculate_results(ig_augmentations, img, np.zeros_like(img_day0))
                heatmap_zero, _, _ = self.calculate_results(ig_augmentations, img, "zero")

                v_long = np.percentile(np.abs(heatmap), 99)
                v_black = np.percentile(np.abs(heatmap_black), 99)
                v_zero = np.percentile(np.abs(heatmap_zero), 99)
                ...
                plt.xticks(np.arange(0, 690, 200))
                ...
                plt.grid()
                plt.tight_layout()
                plt.savefig(scan_figures_folder / f"{scan.apple_nr}_{slice_nr}.svg", dpi=300)
                plt.close()
        file.close()
