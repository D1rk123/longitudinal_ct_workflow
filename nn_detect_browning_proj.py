...
class LRegressionModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        weight_decay = self.trial.suggest_float("weight_decay", 0.05, 0.5, log=True) # value in best trial = 0.1310997746395444
...
class AugmentedProjDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, ff_proj_path, metadata_path, selected_days, day_dict, selected_nrs, ff_proj_mean, ff_proj_std, crop_top, crop_bottom, trial, apply_augmentations=True):
        ...
        self.max_noise_std = self.trial.suggest_float("max_noise_std", 0.002, 0.1, log=True) # value in best trial = 0.0023606876321259442
        self.max_rotate = self.trial.suggest_float("max_rotate", 10, 45) # value in best trial = 12.011658944403864
        self.max_scale = self.trial.suggest_float("max_scale", 1, 1.2) # value in best trial = 1.0874346765326772
        self.max_shear = self.trial.suggest_float("max_shear", 0, 25) # value in best trial = 13.75563849961309
        self.max_translate = self.trial.suggest_int("max_translate", 0, 20) # value in best trial = 0
        self.elastic_alpha_base = self.trial.suggest_float("elastic_alpha_base", 100, 700) # value in best trial = 287.6074334433765
        self.elastic_sigma = self.trial.suggest_float("elastic_sigma", 5, 25) # value in best trial = 14.606329324426024
...
def objective(trial, experiment_folder):
    ...
    crop_top = trial.suggest_int("crop_top", 0, 200) # value in best trial = 131
    crop_bottom = trial.suggest_int("crop_bottom", 0, 200) # value in best trial = 152
...
if __name__ == "__main__":
    ...
    checkpoint_path = get_results_folder() / "2024-03-29_nn_hp_opt_proj_1" / "trial_16" / "checkpoints" / "mae_val" / "best_mae_val_epoch=979_mae_val=0.3602167368.ckpt"
    l_module = LRegressionModule.load_from_checkpoint(checkpoint_path)
    params = l_module.trial.params
    fixed_trial = optuna.trial.FixedTrial(l_module.trial.params)
    del l_module
    
    objective(fixed_trial, experiment_folder)
